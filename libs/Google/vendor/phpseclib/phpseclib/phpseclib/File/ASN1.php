...
abstract class ASN1 {
	 ...
	 * @access private
	 */
	private static $reverseOIDs = array();
	...
	public static function decodeBER( $encoded ) {
		...
		$decoded = array( self::decode_ber( $encoded ) );
	...
	private static function encode_der( $source, $mapping, $idx = null, $special = array() ) {
		...
		switch ( $tag ) {
			...
			case self::TYPE_ANY:
				...
				switch ( true ) {
					case ! isset( $source ):
						return self::encode_der( null, array( 'type' => self::TYPE_NULL ) + $mapping, null, $special );
					case is_int( $source ):
					case $source instanceof BigInteger:
						return self::encode_der( $source, array( 'type' => self::TYPE_INTEGER ) + $mapping, null, $special );
					case is_float( $source ):
						return self::encode_der( $source, array( 'type' => self::TYPE_REAL ) + $mapping, null, $special );
					case is_bool( $source ):
						return self::encode_der( $source, array( 'type' => self::TYPE_BOOLEAN ) + $mapping, null, $special );
					case is_array( $source ) && count( $source ) == 1:
						$typename = implode( '', array_keys( $source ) );
						$outtype  = array_search( $typename, self::ANY_MAP, true );
						if ( $outtype !== false ) {
							return self::encode_der( $source[ $typename ], array( 'type' => $outtype ) + $mapping, null, $special );
						}
				}
