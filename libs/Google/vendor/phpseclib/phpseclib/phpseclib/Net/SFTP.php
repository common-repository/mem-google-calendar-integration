...
class SFTP extends SSH2 {
	...
	private function init_sftp_connection() {
		...
		*/
		...
		$this->update_stat_cache( $this->pwd, array() );
	...
	public function chdir( $dir ) {
		...
		$this->update_stat_cache( $dir, array() );
	...
	private function readlist( $dir, $raw = true ) {
				...
				case NET_SFTP_NAME:
					...
					for ( $i = 0; $i < $count; $i++ ) {
						...
						if ( isset( $attributes['type'] ) && $attributes['type'] == NET_SFTP_TYPE_DIRECTORY && ( $shortname != '.' && $shortname != '..' ) ) {
							$this->update_stat_cache( $dir . '/' . $shortname, array() );
						} else {
							if ( $shortname == '..' ) {
							...
							$this->update_stat_cache( $temp, (object) array( 'lstat' => $attributes ) );
						}
		...
		if ( count( $this->sortOptions ) ) {
			uasort( $contents, array( &$this, 'comparator' ) );
		}
	...
	public function stat( $filename ) {
		...
		if ( isset( $stat['type'] ) ) {
			...
			$this->update_stat_cache( $filename, (object) array( 'stat' => $stat ) );
			return $stat;
		}
	...
	public function lstat( $filename ) {
		...
		if ( isset( $lstat['type'] ) ) {
			...
			$this->update_stat_cache( $filename, (object) array( 'lstat' => $lstat ) );
			return $lstat;
		}
	...
	private function send_sftp_packet( $type, $data, $request_id = 1 ) {
		...
		if ( defined( 'NET_SFTP_LOGGING' ) ) {
			...
			if ( NET_SFTP_LOGGING == self::LOG_REALTIME ) {
				...
				echo $start . $this->format_log( array( $data ), array( $packet_type ) ) . $stop;
				@flush();
				@ob_flush();
			} else {
	...
	private function get_sftp_packet( $request_id = null ) {
		...
		if ( defined( 'NET_SFTP_LOGGING' ) ) {
			...
			if ( NET_SFTP_LOGGING == self::LOG_REALTIME ) {
				...
				echo $start . $this->format_log( array( $packet ), array( $packet_type ) ) . $stop;
				@flush();
				@ob_flush();
			} else {
				$this->packet_type_log[] = $packet_type;
