...
class SSH2 {
	...
	private function key_exchange( $kexinit_payload_server = false ) {
		...
		$kex_algorithms             = isset( $preferred['kex'] ) ?
			$preferred['kex'] :
			self::getSupportedKEXAlgorithms();
		$server_host_key_algorithms = isset( $preferred['hostkey'] ) ?
			$preferred['hostkey'] :
			self::getSupportedHostKeyAlgorithms();
		$s2c_encryption_algorithms  = isset( $preferred['server_to_client']['crypt'] ) ?
			$preferred['server_to_client']['crypt'] :
			self::getSupportedEncryptionAlgorithms();
		$c2s_encryption_algorithms  = isset( $preferred['client_to_server']['crypt'] ) ?
			$preferred['client_to_server']['crypt'] :
			self::getSupportedEncryptionAlgorithms();
		$s2c_mac_algorithms         = isset( $preferred['server_to_client']['mac'] ) ?
			$preferred['server_to_client']['mac'] :
			self::getSupportedMACAlgorithms();
		$c2s_mac_algorithms         = isset( $preferred['client_to_server']['mac'] ) ?
			$preferred['client_to_server']['mac'] :
			self::getSupportedMACAlgorithms();
		$s2c_compression_algorithms = isset( $preferred['server_to_client']['comp'] ) ?
			$preferred['server_to_client']['comp'] :
			self::getSupportedCompressionAlgorithms();
		...
		$kexinit_payload_client .= Strings::packSSH2(
			...
			array(), // language, client to server
			array(), // language, server to client
	...
	private static function mac_algorithm_to_hash_instance( $algorithm ) {
		switch ( $algorithm ) {
			...
			case 'umac-64-etm@openssh.com':
				return array( new Hash( 'umac-64' ), 16 );
			case 'umac-128@openssh.com':
			case 'umac-128-etm@openssh.com':
				return array( new Hash( 'umac-128' ), 16 );
			case 'hmac-sha2-512':
			case 'hmac-sha2-512-etm@openssh.com':
				return array( new Hash( 'sha512' ), 64 );
			case 'hmac-sha2-256':
			case 'hmac-sha2-256-etm@openssh.com':
				return array( new Hash( 'sha256' ), 32 );
			case 'hmac-sha1':
			case 'hmac-sha1-etm@openssh.com':
				return array( new Hash( 'sha1' ), 20 );
			case 'hmac-sha1-96':
				return array( new Hash( 'sha1-96' ), 20 );
			case 'hmac-md5':
				return array( new Hash( 'md5' ), 16 );
			case 'hmac-md5-96':
				return array( new Hash( 'md5-96' ), 16 );
		}
	}
	...
	private function privatekey_login( $username, PrivateKey $privatekey ) {
		...
		$publickeyStr = $publickey->toString( 'OpenSSH', array( 'binary' => true ) );
	...
	private function read_remaining_bytes( $remaining_length ) {
		...
		if ( $remaining_length < -$this->decrypt_block_size || $remaining_length > 0x9000 || $remaining_length % $this->decrypt_block_size != 0 ) {
			if ( ! $this->bad_key_size_fix && self::bad_algorithm_candidate( $this->decrypt ? $this->decrypt->name : '' ) && ! ( $this->bitmap & self::MASK_LOGIN ) ) {
	...
	private function append_log( $message_number, $message ) {
		...
		switch ( NET_SSH2_LOGGING ) {
			...
			case self::LOG_REALTIME:
				...
				echo $start . $this->format_log( array( $message ), array( $message_number ) ) . $stop;
				@flush();
				@ob_flush();
				break;
	...
	public static function getSupportedEncryptionAlgorithms() {
		 ...
		 if ( self::$crypto_engine ) {
			 $engines = array( self::$crypto_engine );
		 } else {
			 $engines = array(
				 'libsodium',
				 'OpenSSL (GCM)',
