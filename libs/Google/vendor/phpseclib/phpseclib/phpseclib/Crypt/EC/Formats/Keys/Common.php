	...
	private static function encodeParameters( BaseCurve $curve, $returnArray = false, array $options = array() ) {
		...
		if ( $useNamedCurves ) {
			if ( isset( self::$curveOIDs[ $name ] ) ) {
				...
				return $returnArray ?
					array( 'namedCurve' => $name ) :
					ASN1::encodeDER( array( 'namedCurve' => $name ), Maps\ECParameters::MAP );
			}
			foreach ( new \DirectoryIterator( __DIR__ . '/../../Curves/' ) as $file ) {
				...
				switch ( $name ) {
					case 'Prime':
						...
						return $returnArray ?
							array( 'namedCurve' => $testName ) :
							ASN1::encodeDER( array( 'namedCurve' => $testName ), Maps\ECParameters::MAP );
					case 'Binary':
		...
		if ( $curve instanceof PrimeCurve ) {
			...
			return $returnArray ?
				array( 'specifiedCurve' => $data ) :
				ASN1::encodeDER( array( 'specifiedCurve' => $data ), Maps\ECParameters::MAP );
		}
