...
class Hash {
	...
	public function setHash( $hash ) {
		...
		if ( in_array( substr( $hash, 0, 5 ), array( 'sha3-', 'shake' ) ) ) {
			...
			if ( version_compare( PHP_VERSION, '7.1.0' ) < 0 || substr( $hash, 0, 5 ) == 'shake' ) {
				...
				$hash             = array( 'phpseclib3\Crypt\Hash', PHP_INT_SIZE == 8 ? 'sha3_64' : 'sha3_32' );
			}
		}
		...
		if ( $hash == 'sha512/224' || $hash == 'sha512/256' ) {
			...
			if ( version_compare( PHP_VERSION, '7.1.0' ) < 0 ) {
				...
				$hash = array( 'phpseclib3\Crypt\Hash', 'sha512' );
			}
		}
